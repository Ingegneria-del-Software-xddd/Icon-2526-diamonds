"""
Modulo per esportazione della Knowledge Base in formato RDF/Turtle.
Integrazione semantica per diamanti secondo standard W3C.
...
import re
# Aggiungi all'inizio del file, insieme agli altri import
...
# =============================================================================
# DEFINIZIONE ONTOLOGIE - STANDARD SEMANTIC WEB
# =============================================================================

# EX: Ontologia personalizzata per il dominio Diamanti
EX = Namespace("http://example.org/diamonds#")

# SCHEMA.ORG: Ontologia standard per descrivere beni di valore
SCHEMA = Namespace("http://schema.org/")

# FOAF: Ontologia per descrivere persone e organizzazioni
FOAF = Namespace("http://xmlns.com/foaf/0.1/")

# QB: Ontologia per dataset statistici
QB = Namespace("http://purl.org/linked-data/cube#")

# DC: Dublin Core per metadati
...
class DiamondRule:
    """
    Rappresentazione intermedia per regole composite sui diamanti.
    
    Bridge tra la KB interna e la rappresentazione RDF semantica.
    """
    name: str
    conditions: List[Tuple[str, str, str]]  # [(feature, operatore, valore), ...]
    beauty_level: str


def slugify(text: str) -> str:
    """
    Converte un testo in formato slug per uso in URI RDF.
    
    Gli URI devono essere URL-safe, consistenti e leggibili.
    
    Args:
        text: Testo da convertire (es: "Diamante Perfetto")
        
    Returns:
        Stringa slug (es: "diamante_perfetto")
    """
    # Converti in minuscolo
    text = text.lower()
    # Sostituisci caratteri non alfanumerici con underscore
    text = re.sub(r'[^a-z0-9]+', '_', text)
    # Rimuovi underscore multipli e quelli iniziali/finali
...
def kb_to_rdf(kb: ExtendedKB) -> Graph:
    """
    Converte una ExtendedKB (con regole per diamanti) in grafo RDF.
    
    Trasforma la struttura dati Python in una rappresentazione semantica
    standardizzata secondo ontologia personalizzata per diamanti.
    
    Args:
        kb: Istanza di ExtendedKB con soglie e regole composite
        
    Returns:
        Graph RDFLib completo con l'intera knowledge base
        
    Note:
        Segue best practices del Semantic Web e Linked Data
    """
    # Crea grafo RDF vuoto
    g = Graph()
    
    # REGISTRA PREFIXES per serializzazione leggibile
    g.bind("ex", EX)           # Diamonds ontology (custom)
    g.bind("schema", SCHEMA)   # Schema.org (standard per beni)
    g.bind("foaf", FOAF)       # Friend of a Friend (agenti)
    g.bind("qb", QB)           # Data Cube (dataset statistici)
    g.bind("dc", DC)           # Dublin Core (metadati)
    g.bind("xsd", XSD)         # XML Schema Datatypes

    # =========================================================================
    # DEFINIZIONE CLASSI ONTOLOGIA (TBox - Terminological Box)
    # =========================================================================
    
    # Classi principali del dominio Diamanti
    ...
    # Sottoclassi di DiamondFeature per categorie specifiche
    ...
    # Gerarchia: le sottoclassi sono sottoclassi di DiamondFeature
    ...
    # =========================================================================
    # ENUMERAZIONE LIVELLI DI BELLEZZA (Instances)
    # =========================================================================
    
    for level in ["LOW", "MEDIUM", "HIGH"]:
        level_uri = EX[level]
        g.add((level_uri, RDF.type, EX.BeautyLevel))      # Istanza della classe
        g.add((level_uri, RDFS.label, Literal(level)))    # Etichetta leggibile
        g.add((level_uri, DC.description, Literal(f"Livello di bellezza {level}")))
        
        # Aggiungi descrizioni specifiche per ogni livello
        if level == "LOW":
    ...
    # =========================================================================
    # DEFINIZIONE PROPRIETÀ (Properties Box)
    # =========================================================================
    
    # Proprietà per descrivere le soglie di qualità
    ...
    # Proprietà per regole composite
    ...
    # Proprietà generali per diamanti
    ...
    # Proprietà per dataset/mapping
    g.add((EX.mapsToDatasetColumn, RDF.type, RDF.Property))

    # =========================================================================
    # POPOLAMENTO FEATURE E SOGLIE (ABox - Assertional Box)
    # =========================================================================
    
    # Mappatura feature -> categoria RDF
    feature_categories = {
        ...
        'price': EX.DiamondFeature  # Feature generica
    }
    
    # Per ogni threshold nella KB
    for position, threshold in kb._store.items():
        # Crea URI per la feature (es: http://example.org/diamonds#carat)
        feature_name = threshold.feature
        feature_uri = EX[feature_name]
        
        # Definisci il tipo di feature in base alla categoria
        ...
        # Aggiungi proprietà di categoria
        if "carat" in feature_name:
            g.add((feature_uri, EX.featureCategory, Literal("weight")))
            g.add((feature_uri, SCHEMA.unitCode, Literal("CT")))  # Codice unità: carati
            ...
            g.add((feature_uri, SCHEMA.unitCode, Literal("MM")))  # Millimetri

        # Crea nodo soglia per questa feature
        threshold_uri = URIRef(f"{EX}threshold_{feature_name}_{position}")
        g.add((threshold_uri, RDF.type, EX.QualityThreshold))
        
        # Aggiunge proprietà della soglia
        ...
        # Collega feature alla sua soglia
        g.add((feature_uri, EX.hasThreshold, threshold_uri))

    # =========================================================================
    # REGOLE COMPOSITE (Logica di business complessa)
    # =========================================================================
    
    # Per ogni regola composta nella ExtendedKB
    for rule in getattr(kb, "composite_rules", []):
        # Crea URI per la regola
        ...
        # Aggiunge ogni condizione della regola
    ...
    # =========================================================================
    # AGGIUNTA METADATI DELLA KNOWLEDGE BASE
    # =========================================================================
...
def save_kb_to_rdf(kb: ExtendedKB, output_path: str = "diamonds_kb.ttl") -> str:
    """
    Salva una ExtendedKB in formato RDF/Turtle.
    
    Args:
        kb: Knowledge Base da esportare
        output_path: Percorso del file di output (.ttl)
        
    Returns:
        Percorso del file generato
    """
    # Converti KB in grafo RDF
    g = kb_to_rdf(kb)
    
    # Serializza in formato Turtle (leggibile e compatto)
...
def load_kb_from_rdf(rdf_path: str) -> ExtendedKB:
    """
    Carica una Knowledge Base da file RDF/Turtle.
    
    Processo inverso: da RDF semantico a struttura operativa Python.
    
    Args:
        rdf_path: Percorso al file RDF/Turtle
        
    Returns:
        ExtendedKB ricostruita dal file RDF
    """
    # Carica grafo RDF
    g = Graph()
    g.parse(rdf_path, format="turtle")
    
    # Crea nuova ExtendedKB
    kb = ExtendedKB()
    ...
    # =========================================================================
    # ESTRAZIONE REGOLE COMPOSITE
    # =========================================================================
    
    # Cerca tutte le regole composite
...
def generate_diamond_rdf_report(
    ...
    """
    Genera un report RDF per la valutazione di un diamante specifico.
    
    Crea una rappresentazione semantica completa di:
    - Il diamante valutato
    - Le caratteristiche osservate
    - Le soglie violate/rispettate
    - Il punteggio fuzzy calcolato
    
    Args:
        diamond: Dizionario con caratteristiche del diamante
        kb: Knowledge Base usata per la valutazione
    ...
    # Crea URI per questo diamante specifico
    ...
    # Aggiungi tutte le caratteristiche osservate
    for feature, value in diamond.items():
        if feature != "id":  # Skip ID se presente
            feature_uri = EX[feature]
            g.add((diamond_uri, EX.hasFeature, feature_uri))
            
            # Nodo per il valore osservato
    ...
    # Aggiungi punteggio come proprietà
    ...
    for position, thr in kb._store.items():
        if thr.feature in diamond:
            ...
            # Valuta se la soglia è rispettata
            # (Semplificato - implementazione completa richiederebbe logica di valutazione)
...
def query_rdf_kb(
    ...
    """
    Opzione nucleare: converte tutto in JSON e poi in dict.
    Funziona SEMPRE ma potrebbe essere lenta.
    
    Args:
        rdf_path: Percorso al file RDF
        sparql_query: Query SPARQL da eseguire
        fallback_simple: Se True, in caso di errore prova approccio semplice
    ...
    try:
        # 1. Carica il file RDF
        g.parse(rdf_path, format="turtle")
        
        # 2. Esegui query
        qres = g.query(sparql_query)
        
        # 3. Serializza in JSON (RDFLib >= 4.0 supporta 'json')
        ...
        # 5. Estrai risultati dal formato SPARQL JSON standard
        # Formato standard: {"results": {"bindings": [{...}, ...]}}
        if 'results' in json_data and 'bindings' in json_data['results']:
            for binding in json_data['results']['bindings']:
                ...
                for key, value_info in binding.items():
                    # value_info è un dict con 'type' e 'value'
                    # Es: {"type": "uri", "value": "http://..."}
                    row_dict[key] = value_info.get('value', '')
                results.append(row_dict)
        
        # Alternative: gestione di altri formati JSON
        elif 'boolean' in json_data:
            # Per query ASK (true/false)
        ...
        # IMPORTANTE: NO ricorsione infinita!
        if fallback_simple:
            # Fallback a un approccio semplice ma non ricorsivo
...
def query_rdf_kb_fallback(
    ...
    """
    Fallback semplice senza ricorsione.
    """
    from rdflib import Graph
    ...
    try:
        ...
        # Approccio ultra-semplice: itera e converte
        for row in qres:
            row_dict = {}
            
            # Se row è iterabile (non bool)
            if isinstance(row, bool):
                # Per query ASK che ritornano bool
                row_dict["result"] = str(row)
            elif hasattr(row, '__iter__') and not isinstance(row, str):
                # Prova a trattarlo come sequenza
                try:
                    for i, item in enumerate(row):
                        row_dict[f"var_{i}"] = str(item) if item is not None else ""
                except TypeError:
                    # Fallback se l'iterazione fallisce
                    row_dict["value"] = str(row) if row is not None else ""
            else:
                # Altrimenti converto tutto in stringa
                row_dict["value"] = str(row) if row is not None else ""
...
# =============================================================================
# QUERY SPARQL PREDEFINITE UTILI
# =============================================================================
...
def export_kb_with_ml_integration(
    ...
    """
    Esporta una Knowledge Base integrata con informazioni del modello ML.
    
    Crea un ecosistema semantico completo che include:
    - Regole di dominio (KB)
    - Informazioni del modello ML
    - Metadati del dataset
    - Performance del sistema
    
    Args:
        kb: Knowledge Base con regole
        model_info: Dizionario con informazioni del modello ML
        output_base: Base per i nomi dei file di output
    ...
    # 2. Crea grafo per informazioni modello ML
    g_model = Graph()
    g_model.bind("ex", EX)
    ...
    # Aggiungi proprietà del modello
    ...
    # 3. Integra i due grafi
    g_kb = Graph()
    g_kb.parse(kb_ttl, format="turtle")
    
    g_kb += g_model  # Unisci i grafi
    
    # Aggiungi relazioni tra KB e modello
